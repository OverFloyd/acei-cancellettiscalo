...
## 5b, 6a, 7b
### Set buffer
execute store result score Cancelletti buffer if block -116 71 17 rail[shape=east_west] if block -122 71 17 rail[shape=east_west] if block -133 70 41 rail[shape=north_south]

## Anomalia
function cancellettiscalo:apparato/logica/anomalie/dev

# BLOCCAMENTO PUNTO ORIGINE 2
## Check buffer
execute if score Cancelletti buffer matches 1 run function cancellettiscalo:apparato/ql/punti/2_bloc